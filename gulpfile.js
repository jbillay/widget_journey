...
gulp.task('wordpress-package', ['wordpress-plugin'], function () {
    ...
    return gulp.src(['dist/wordpress/**', '!dist/wordpress/mrt_widget_journey.zip'])
        .pipe(zip('mrt_widget_journey_wordpress_' +
...
gulp.task('joomla-package', ['joomla-plugin'], function () {
    ...
    return gulp.src(['dist/joomla/**', '!dist/joomla/mrt_widget_journey.zip'])
        .pipe(zip('mrt_widget_journey_joomla_' +
                    process.env.NODE_ENV + '_' +
                    d.getFullYear() +
                    ('00' + (d.getMonth() + 1)).substr(-2) +
                    ('00' + d.getDate()).substr(-2) +
...
gulp.task('html-package', ['build'], function () {
    var d = new Date();
    return gulp.src(['index.html', 'dist/**', '!dist/mrt_widget_journey.zip'])
        .pipe(zip('mrt_widget_journey_html_' +
        process.env.NODE_ENV + '_' +
        d.getFullYear() +
        ('00' + (d.getMonth() + 1)).substr(-2) +
        ('00' + d.getDate()).substr(-2) +
